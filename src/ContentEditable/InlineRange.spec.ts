...
const sampleHTML =
  '<p id="host"><strong>Hello!</strong> This is ' +
...
it('tests wrappers', () => {
  ...
  const forEveryTwo = (x, y) => {
    ...
    const actual1 = InlineRange.fromNodes(host, x.node, x.offset, y.node, y.offset)
    const actual2 = InlineRange.fromStaticRange(host, {
      startContainer: x.node,
      startOffset: x.offset,
      endContainer: y.node,
      endOffset: y.offset,
      collapsed: x.node === y.node && x.offset === y.offset
    })
    const actual3 = InlineRange.fromPaths(
      host,
      pathFromChild(host, x.node),
      x.offset,
      pathFromChild(host, y.node),
      y.offset
    )
